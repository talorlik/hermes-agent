    ...
    with _board_conn(board) as (board, conn):
        ...
        with kanban_db.read_txn(conn):
            snapshot = kanban_db.build_task_snapshot(
                conn, task_id, run_state_type=run_state_type, run_state_name=run_state_name)
            if snapshot is None:
                raise HTTPException(status_code=404, detail=f"task {task_id} not found")
            serialized = snapshot.to_dict()
            # Preserve dashboard-only derived fields while sourcing canonical task fields from
            # the shared versioned snapshot serializer. Drawer returns the FULL summary (cards
            # on /board carry a 200-char preview).
            task_d = _task_dict(snapshot.task, latest_summary=snapshot.latest_summary)
            task_d.update(serialized["task"])
            links = _links_for(conn, task_id)
            child_summaries = kanban_db.latest_summaries(conn, snapshot.children)
            children = filter(None, (kanban_db.get_task(conn, cid) for cid in snapshot.children))
            _attach_diagnostics(task_d, _compute_task_diagnostics(conn, task_ids=[task_id]).get(task_id) or [])
            return {
                "schema_version": snapshot.schema_version,
                "task": task_d,
                "comments": [asdict(c) for c in snapshot.comments],
                ...
                "child_results": [
                    ...
                    for c in children],
                "runs": [asdict(r) for r in snapshot.runs]}
