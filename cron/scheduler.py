...
from cron.executions import (
    ...
    mark_execution_handoff_pending, mark_execution_running, record_delivery,
...
def _run_no_agent_job(
    ...
    _script_outcome = classify_script_result(
        ok, output, getattr(_script_result, "returncode", None),
        occurrence_key=job_occurrence_key(job))
    if _script_outcome.kind == TRANSIENT_DEFER:
        # A defer is neither success nor failure: no watchdog alert, no failure streak, no
        # consumed occurrence. The typed marker tells the shared firing body to persist a
    ...
    if _script_outcome.kind == DETACHED:
        # RUN_STARTED: the script launched a detached worker that owns finishing this
        # occurrence; the firing body leases the execution to it instead of terminalizing.
...
def _apply_monitor_gate(
    ...
    pending_monitor_commit)``; when early_result is None, extra_prompt may carry the injected
    monitor context and ``pending_monitor_commit`` carries a detected change whose dedup hash the
    caller commits AFTER preflight succeeds (a blocked/crashed run stays retryable).
    """
    ...
    # The dedup hash is NOT committed here — it commits after preflight succeeds, right before
    # the agent runs, so a blocked/crashed run leaves this change retryable.
    if _mon.context_block:
        extra_prompt = (
            f"{_mon.context_block}\n\n{extra_prompt}" if extra_prompt else _mon.context_block
        )
    return None, extra_prompt, _mon
...
def _deferred_script_result(
    ...
    success=False keeps the tuple contract honest (the run did not produce a result), but the
    typed ``CronPreScriptDefer`` error is what the shared firing body dispatches on — a defer
    must never reach the failure-alert, incident, or failure-streak paths.
    """
...
def _detached_script_result(
    ...
    success=True — the launch succeeded — but nothing is delivered and the typed
    ``CronDetachedStart`` marker tells the shared firing body to lease the execution to the
    worker instead of terminalizing it.
    """
    ...
    logger.info(
        ...
        job_name, job_id, detached.run_id, detached.lease_seconds, detached.worker or None)
    doc = (
        f"# Cron Job: {job_name}\n\n"
        f"**Job ID:** {job_id}\n"
        f"**Run Time:** {now_iso}\n"
...
def _fail_closed_script_result(
    ...
    try:
        ...
        script_error = redact_sensitive_text(
            str(script_output or "Script failed"), force=True, redact_url_credentials=True)
    except Exception:
...
def _prepare_job_prompt(
    ...
    pending_monitor_commit)``: an early result short-circuits ``run_job`` (no_agent job, empty
    payload, monitor gate, wake gate, injection block, empty prompt); otherwise ``prompt`` is set
    and ``pending_monitor_commit`` may carry a monitor change to commit after preflight."""
    # Stored policy is an execution control. Only an absent key receives the legacy default;
    # malformed explicit values fail before any script or agent side effect can run.
    ...
    _early, extra_prompt, _pending_monitor_commit = _apply_monitor_gate(
    ...
    if script_path:
        ...
        _prerun_outcome = classify_script_result(
            _ran_ok, _script_output, getattr(prerun_script, "returncode", None),
            occurrence_key=job_occurrence_key(job))
        if _prerun_outcome.kind == TRANSIENT_DEFER:
            # Defer outranks BOTH script-failure policies: fail_closed would misreport contention
            # as a broken automation, and continue would wake the agent on an occurrence that
            # must be retried intact.
            return _deferred_script_result(
                job_id, job_name, _prerun_outcome,
                _hermes_now().strftime("%Y-%m-%d %H:%M:%S")), None, None
        if _prerun_outcome.kind == DETACHED:
            # The detached worker owns this occurrence — waking the agent on top of it would
            # double-run the work.
    ...
    return None, prompt, _pending_monitor_commit
...
def run_job(
    ...
    early, prompt, _pending_monitor_commit = _prepare_job_prompt(
    ...
    try:
        ...
        if _pending_monitor_commit is not None:
            # Provider/model/tool preflight passed and the durable work intent (execution row)
            # exists — commit the monitor dedup hash now, BEFORE the agent runs, so an agent
            # failure does not re-alert forever while a preflight failure stayed retryable.
...
def _register_detached_start(
    job: dict, error: "CronDetachedStart", fire_owner: Optional[str], execution_id: str,
) -> bool:
    """DETACHED (RUN_STARTED): lease the execution to the worker in one atomic write and KEEP IT
    NONTERMINAL — the worker finalizes by correlation id (`hermes cron finalize-detached`) and
    the tick's reconcile sweep writes the terminal state. The job record advances (claims
    released, occurrence handed off), but success-side bookkeeping — incident recovery, deferral
    resolution — waits for the real terminal outcome at reconciliation."""
    ...
    if fire_owner is not None and not marked:
        # Ownership was lost AFTER the worker launched — the lease stands regardless (the worker
        # runs either way); reconciliation settles the truth. Only the job-record projection is
        # skipped, and visibly so.
        logger.warning(
            "Job '%s': fire claim ownership lost after detached registration of run %s; "
            "job-record update skipped",
            job["id"], error.run_id)
    return True


def _apply_pre_script_defer(
    job: dict, error: "CronPreScriptDefer", fire_owner: Optional[str], execution_id: str,
) -> tuple[bool, object]:
    """TRANSIENT_DEFER: neither success nor failure. Persist the durable retry obligation, project
    it onto the job record (claims released, occurrence NOT consumed), and terminate this attempt
    as 'deferred' in the ledger. Returns ``(handled, error)``: when the retry budget is already
    spent the obligation exhausts, ``handled`` is False and ``error`` is the permanent-failure
    text for the NORMAL failure machinery (incident + delivery + schedule advance)."""
    ...
    if not deferred_marked and fire_owner is not None:
        # The fenced handoff failed AFTER the obligation persisted: compensate deterministically
        # so the replacement owner never inherits (and can never exhaust) this stale row.
...
def _resolve_pending_deferral_safe(job_id: str, outcome: str) -> None:
    """A real terminal outcome resolves any pending deferred obligation: completed occurrences pay
    the debt, failures settle it permanently. Best-effort — never breaks the run path."""
    try:
        from cron.deferrals import resolve_pending

        resolve_pending(job_id, outcome)
    except Exception:
        logger.debug("Failed resolving deferral for job %s", job_id, exc_info=True)
...
def _compose_run_delivery(
    ...
    # A scheduler-owned typed marker selects the fail-closed script branch before any string
    # markers or provider heuristics can inspect untrusted script text.
    ...
    blocked_config = blocked_config_silent or (not script_failure and BLOCKED_CONFIG_MARKER in err)
    drift_skip_silent = not script_failure and DRIFT_SKIP_SILENT_MARKER in err
    drift_skip = drift_skip_silent or (not script_failure and DRIFT_SKIP_MARKER in err)
    incident_acked = False
    failure_incident_id = None
    if script_failure and not success:
        # Script stderr/stdout is untrusted data saved only in the output document.
...
def _save_compose_deliver(
    ...
    if not d.should_deliver:
        return
    _lane_value = _normalize_deliver_value(_delivery_lane_value(job, for_failure=not d.success))
    d.unresolved_origin = (
        _lane_value == "origin"
        and not _resolve_delivery_targets(job, for_failure=not d.success)
    )
    # Durable outbox: persist the pending delivery + the execution's terminal intent atomically
    # BEFORE the send, so a crash mid-send leaves retryable evidence instead of a silent loss.
    # Local-only jobs have no platform send to make durable. Best-effort: outbox bookkeeping must
    # never break the delivery path itself.
    _outbox_entry = None
    if execution_id is not None and _lane_value != "local" and not d.unresolved_origin:
        try:
            ...
            _outbox_entry = enqueue_with_intent(
                ...
                intent_error=None if d.success else str(d.error or ""))
        except Exception:
    ...
    if _outbox_entry is not None:
        try:
            ...
            logger.warning("Outbox attempt record failed for job %s", job["id"], exc_info=True)
    if execution_id is not None and _lane_value != "local":
        try:
            record_delivery(
                execution_id,
                target=_lane_value,
                status=(
                    "not_configured" if d.unresolved_origin
                    else "failed" if d.delivery_error
                    else "delivered"),
                error=d.delivery_error)
        except Exception:
...
def _finish_completed_run(d: _RunDelivery, fire_owner: Optional[str], execution_id: str) -> bool:
    ...
    if fire_owner is not None and not marked:
        ...
        return True
    _resolve_pending_deferral_safe(job["id"], "completed" if d.success else "permanent")
    if d.success:
        # The failure healed without operator action — open incidents for this job flip to
        # 'recovered' (acked ones stay closed). Best-effort.
        try:
            ...
            logger.debug(
                "Failed recording incident recovery for job %s", job["id"], exc_info=True)
    ...
    if delivery_outcome == "delivered" and not d.success:
        # The failure ping actually left the process — record it on the incident so the CLI
        # distinguishes "failure seen" from "operator was pinged". A not_configured outcome
        # composed an alert nobody received: the incident stays in its truthful undelivered
        # state ('detected') so operators still see it open.
...
def _deliver_crash_failure(
    ...
    # the nudge must leave through here too (#88655).
    _alert_content = (
    ...
    # SAME durable protocol as the normal delivery path: persist the pending alert + this
    # execution's failure intent atomically BEFORE the send so a crash mid-alert leaves
    # retryable, queryable evidence. Best-effort — outbox bookkeeping must never break the alert.
    _crash_outbox_entry = None
    if execution_id is not None and normalized_deliver != "local" and not unresolved_origin:
        try:
            from cron.outbox import enqueue_with_intent
    ...
    if _crash_outbox_entry is not None:
        try:
            ...
            logger.warning("Outbox attempt record failed for job %s", job["id"], exc_info=True)
    if execution_id is not None and normalized_deliver != "local":
        try:
            record_delivery(
                execution_id,
                target=normalized_deliver,
    ...
    if delivery_outcome == "delivered":
        # Same truthful-state contract as the normal failure path: not_configured never reached
        # anyone, so the incident must not claim the operator was pinged.
...
def _run_one_job_body(
    ...
    try:
        ...
        if isinstance(error, CronDetachedStart) and success:
            # No delivery for a RUN_STARTED launch — the worker owns the occurrence.
            _teardown_deferred()
            return _register_detached_start(job, error, fire_owner, execution_id)
        if isinstance(error, CronPreScriptDefer) and not success:
            _defer_handled, error = _apply_pre_script_defer(job, error, fire_owner, execution_id)
            if _defer_handled:
                _teardown_deferred()
                return True
            # Exhausted retry budget: fall through to the NORMAL permanent-failure machinery
            # (incident + delivery + schedule advance) with the exhausted-budget error text.
        ...
        try:
            if not _consume_interrupted_flag(job["id"], execution_token):
                ...
                _resolve_pending_deferral_safe(job["id"], "permanent")
...
def _retry_pending_deliveries(adapters=None, loop=None, max_rows: int = 10) -> int:
    ...
    Runs at the top of every tick, BEFORE new work: a recovered channel replays its backlog
    first. Every attempt (success or failure) lands in the per-attempt history; a job that no
    longer exists abandons its entries. Fully best-effort — outbox trouble must never stall the
    tick.
    """
    try:
    ...
    for entry in backlog:
        try:
            ...
            logger.debug("Outbox retry failed for entry %r", entry.get("id"), exc_info=True)
    return delivered
...
def _maybe_reap_dead_owners() -> None:
    ...
    try:
        from cron.executions import (
            reconcile_detached_runs, recover_interrupted_executions)

        # Detached terminal reports (and expired leases) settle BEFORE the dead-owner sweep so a
        # finished detached run is never misread as an abandoned attempt.
        _settled = reconcile_detached_runs()
        if _settled:
            logger.info("Reconciled %d detached cron run(s)", len(_settled))
...
def _process_due_job(job: dict, adapters, loop, verbose: bool) -> bool:
    ...
    # The claimed record's next_run_at is already advanced; the logical occurrence travels with
    # the dispatched dict.
    claimed_job["due_occurrence_at"] = job.get("due_occurrence_at") or job.get("next_run_at")
...
def _submit_with_guard(job: dict, pool: concurrent.futures.ThreadPoolExecutor, process_job):
    ...
    try:
        ...
        # Freeze the logical occurrence NOW: the due-scan dict still holds the scheduled fire
        # time, while advance_next_runs (above) and claim_job_for_fire (later) both rewrite the
        # stored next_run_at. Deferred obligations dedupe on this key.
        dispatched_job = dict(
            job, execution_id=execution["id"], due_occurrence_at=job.get("next_run_at"))
...
def tick(
    ...
    try:
        ...
        # At-least-once delivery: drain the pending outbox backlog BEFORE any new work is
        # dispatched, so a healed channel replays what it missed ahead of producing more.
