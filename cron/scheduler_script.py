...
def _resolve_script_path(script_path: str) -> tuple[Optional[Path], Optional[str]]:
    ...
    try:
        ...
        return None, "Blocked: Hermes scripts directory is unavailable"
...
def _redact_job_script_result(success: bool, output: object) -> tuple[bool, str]:
    ...
    try:
        ...
        # Do not log through the redacting formatter here. If the redactor is unavailable, that
        # would recursively invoke the same failing boundary and could expose the exception
        # instead of returning the safe constant.
