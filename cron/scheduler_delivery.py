...
def _target_mirror_eligible(
    job: dict, target: dict, *, global_mirror: bool, origin_match: Optional[bool] = None
) -> bool:
...
def _open_continuable_cron_thread(
    job: dict, adapter, chat_id: str, loop
) -> Optional[str]:
...
def cron_delivery_targets() -> list[dict]:
    ...
    try:
        ...
        logger.debug(
            "cron_delivery_targets: profile listing unavailable", exc_info=True
        )
    return targets
...
def _origin_thread_is_stale(origin: dict) -> bool:
    ...
    if str(origin.get("platform") or "").lower() != "slack" or not origin.get(
        "thread_id"
    ):
        return False
...
def _home_target(
    platform_name: str, chat_id: str, resolved_from: Optional[str] = None
) -> dict:
...
def _resolve_single_delivery_target(
    ...
    if ":" in deliver_value:
        ...
        if resolution_error:
            logger.warning(
                "Invalid cron delivery target '%s': %s", deliver_value, resolution_error
            )
            return None
        if (
            thread_id is None
...
def _deliver_to_bot_chat(job: dict, content: str, profile: str) -> Optional[str]:
    ...
    try:
        ...
        receipt_identity = [str(source_home), job_id, str(run_id), str(home)]
        exact_attempt_id = job.get("_exact_delivery_attempt_id")
        if exact_attempt_id is not None:
            receipt_identity.append(str(exact_attempt_id))
        key = hashlib.sha256(
            json.dumps(
                receipt_identity,
                ensure_ascii=False,
        ...
        if receipt is not None:
            ...
            logger.info(
                "Job '%s': Bot Chat %s receipt=%s status=%s",
                job_id,
                profile_label,
                key,
                status,
            ...
            if status == "settled":
                return None
            detail = (
                "completion unverified; do not resend"
    ...
    try:
        ...
        return _fail(
            f"bot-chat delivery failed: {str(e) or type(e).__name__}", exc_info=True
        )
    finally:
        if query_file:
...
def _resolve_delivery_targets(job: dict, *, for_failure: bool = False) -> List[dict]:
    ...
    deliver = _normalize_deliver_value(
    ...
    for raw in deliver.split(","):
        ...
        for part in _expand_routing_tokens(raw):
            ...
            target["thread_id"] = str(thread_id) if thread_id is not None else None
            key = (
    ...
    for media_path, _is_voice in media_files:
        try:
            ...
            if future is None:
                _note_target_error(
                    job_ref,
                    f"cannot send media {media_path}: gateway loop unavailable",
                    errors,
                )
                return errors
            try:
...
def _confirm_adapter_delivery(
    ...
    * No ``message_id`` and no ``raw_response`` means we have no positive evidence of a send. Telegram
    ...
    if _result_field(send_result, "delivered") is False:
        return False
    if _result_field(send_result, "message_id") is None and not _result_field(
...
def _is_channel_dm_topic(
    runtime_adapter: Any, chat_id: Any, loop: Any, job_id: str
) -> bool:
...
def _cron_delivery_notify_enabled(cfg: Optional[dict]) -> bool:
    ...
    try:
        ...
        return (
            not isinstance(delivery_cfg, dict)
            or delivery_cfg.get("notify", True) is not False
        )
    except Exception:
        return True
...
def _record_delivery_verification(job: dict, unverified_targets: list) -> None:
    ...
    queued = {
        target: receipt
        for target, receipt in job.get("_bot_chat_delivery_receipts", {}).items()
        if receipt["status"] in ("queued", "claimed")
    } or None
    projection = {
        "last_delivery_unverified": new_value,
        "last_delivery_queued": queued,
    }
    values = {
        key: value
        for key, value in projection.items()
        if (job.get(key) or None) != value
    }
    if not values:
        return
    job.update(values)
    projection_revision = job.get("_delivery_projection_revision")
    execution_id = job.get("execution_id")
    if projection_revision is None or execution_id is None:
        return
    try:
...
def _live_send_text(
    ...
    *,
    ...
    if send_raw_response and t.thread_id and send_raw_response.get("thread_fallback"):
        requested_thread_id = (
            send_raw_response.get("requested_thread_id") or t.thread_id
        )
        _note_target_error(
            job,
    ...
    *,
    ...
    try:
        ...
        if not text_to_send and not media_files:
            ...
            _note_target_error(
                job,
                f"live adapter send skipped (empty text and no media) for {t.where}",
                target_errors,
            )
            adapter_ok = False
        elif text_to_send:
...
def _standalone_send(
    t: _TargetDelivery, content: str, media_files: list
    ...
    if not content.strip() and not media_files:
        return _warned(
            f"standalone send skipped (empty text and no media) for {t.where}"
        )
    coro = _send()
    try:
...
def _prepare_target_delivery(
    ...
    *,
    ...
    in_channel_surface = (
        _resolve_cron_surface_mode(pconfig, platform_name) == "in_channel"
    )
    if (
        in_channel_surface
    ...
    if (
        ...
        opened_thread_id = (
            _open_continuable_cron_thread(job, runtime_adapter, chat_id, loop) or None
        )
        if opened_thread_id:
...
def _deliver_result(
    ...
    *,
    ...
    """Deliver exact or resolved targets and finalize only a caller-owned run."""
...
def _deliver_resolved_targets(
    ...
    *,
    ...
    loop=None,
    for_failure: bool,
    allow_worker_queue: bool,
    outbox_id: Optional[str] = None,
    delivery_run: DeliveryRunAccumulator,
) -> Optional[str]:
    """Deliver concrete targets through the current adapter and credential stack."""
    ...
    if (
        ...
        delivery_run.record(
            DestinationDeliveryOutcome(
                delivery_id=str(outbox_id or external_execution),
                target=targets[0],
    ...
    for target_index, target in enumerate(targets):
        delivery_id = str(
            outbox_id
            or f"legacy:{target_index}:{target.get('platform')}:{target.get('chat_id')}"
        )
        target_error_start = len(delivery_errors)
        target_unverified: list[str] = []
        try:
            ...
            if target["platform"] == BOT_CHAT_PLATFORM:
                ...
                if bot_chat_error:
                    ...
                    if receipt and receipt["status"] == "ambiguous":
                        target_unverified.append(bot_chat_error)
                continue
            ...
            if t.opened_thread_id and outbox_id is not None:
                ...
                try:
                    ...
                    _note_target_error(
                        job,
                        f"could not persist continuation thread for {t.where}: {exc}",
                        delivery_errors,
                    )
                    continue
            target_errors: list = []
            ...
            receipt_items: tuple[tuple[str, dict], ...] = ()
            if target["platform"] == BOT_CHAT_PLATFORM:
                receipt_target = f"bot-chat:{target['chat_id'] or '(own)'}"
            ...
            delivery_run.record(
                DestinationDeliveryOutcome(
                    ...
                    bot_chat_receipts=receipt_items,
                    unverified_targets=tuple(target_unverified),
