...
def create_job(
    ...
    delivered verbatim, requires ``script``). script_failure_policy: ``continue`` injects a failed
    pre-run script's error into the prompt (legacy default); ``fail_closed`` fails the run before
    the agent starts and requires ``script``. context_from: job id(s) whose latest output is
    ...
    def apply(jobs, i, job):
        ...
        if {
            ...
            _validate_job_mode_invariants(
                ...
                _normalize_script_failure_policy(updated.get("script_failure_policy")))
...
def _record_run_outcome(
    ...
    # A terminal run resolves any deferred occurrence either way (the durable obligation row in
    # cron.deferrals keeps the history); the job-record projection must not outlive it.
