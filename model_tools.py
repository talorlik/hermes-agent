...
#
# Plugin tool discovery (user/project/pip plugins) is skipped under the
# explicit-no-tools process guard set by `hermes -z --toolsets none` (see
# hermes_cli.oneshot._EXPLICIT_NO_TOOLS_ENV): that run builds an agent with
# enabled_toolsets=[], so no plugin registration could ever be consulted and
# import-time discovery would be a pure startup side effect. Any value other
# than exactly "1" keeps normal behavior.
