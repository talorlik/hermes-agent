...
def _load_tools(agent, enabled_toolsets, disabled_toolsets):
    # An explicit empty toolset list (hermes -z --toolsets none) is a no-tools
    # contract, distinct from None (default/config toolsets): no plugin
    # registration could ever be consulted, so plugin discovery is skipped, the
    # tool snapshot is pinned to [] without consulting the registry (a
    # dispatcher-context kanban append must not resurrect tools), and the
    # between-turns MCP refresh is opted out so a late MCP registration cannot
    # re-introduce tools mid-session.
