...
def _handle_show(args: dict, **kw) -> str:
    """Full task state: pinned snapshot of row, parents, children, comments, runs, last 50
    events."""
    tid = _require_task_id(args)
    with _board(args.get("board")) as (kb, conn):
        with kb.read_txn(conn):
            ...
            # Model-tool consumer cap only: the last 50 events keep the tool output bounded
            # (full log via CLI / raw snapshot). No truncation metadata -- exact prior shape.
            payload["events"] = payload["events"][-50:]
            # Preserve the model-tool-specific worker context while deriving it from the same
            # pinned SQLite snapshot (same string build_worker_context hands the dispatcher).
