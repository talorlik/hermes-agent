...
async def _send_telegram(token, chat_id, message, media_files=None, thread_id=None, disable_link_previews=False, force_document=False):
    ...
    try:
        ...
        text_chunks = BasePlatformAdapter.truncate_message(formatted, 4096, len_fn=utf16_len) if formatted.strip() else []
        if len(text_chunks) > 1 and not _has_html:  # not HTML => MarkdownV2 (see _telegram_format)
            # truncate_message appends a raw " (1/2)" suffix whose bare parentheses are
            # reserved in MarkdownV2, so Telegram rejects the chunk and the whole message
            # falls back to plain text. Mirror the gateway adapter's escaping (adapter.send
            # path) so multi-chunk sends keep their formatting.
            try:
