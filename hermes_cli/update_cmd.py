...
from hermes_cli.update_cmd_git import (  # noqa: F401
    ...
    _sync_with_upstream_if_needed, _FORK_SYNC_TEST_PATHS, _fork_sync_strategy,
    _merge_upstream_into_fork, _rollback_fork_sync_candidate, _run_fork_sync_tests,
    _subprocess_detail, _validate_fork_sync_candidate)
...
class _CheckoutPlan:
    ...
    # True when the fork upstream sync (updates.fork_sync_strategy) already moved
    # HEAD and pushed it to origin before the pull; the pull is then a legitimate
    # no-op and must not trip the "Code did not move" guard.
    upstream_sync_moved_head: bool = False
...
def _verify_head_after_pull(
    ...
    # claiming success. A fork upstream sync (updates.fork_sync_strategy) can
    # legitimately move HEAD and push that commit to origin before this pull,
    # making the pull a no-op even though the overall update did move the checkout.
...
def _apply_pulled_update(
    ...
    post_pull_sha = _verify_head_after_pull(
        ...
        _windows_gateway_resume=_windows_gateway_resume,
        upstream_sync_moved_head=getattr(_plan, "upstream_sync_moved_head", False))
