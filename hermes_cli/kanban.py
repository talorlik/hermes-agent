...
from hermes_cli.kanban_output import (
    _ATTACHMENT_FIELDS, _RUNS_RUN_FIELDS, _bulk_apply, _err,
...
from hermes_cli.kanban_parser import _positive_run_id
...
def _cmd_show(args: argparse.Namespace) -> int:
    ...
    with kbc.connect_closing() as conn:
        # One consistent read: task row, links, comments, events, runs and the
        # latest summary come from a single pinned snapshot, so a concurrent
        # claim cannot produce a ``ready`` row alongside its own new run.
        snap = kb.build_task_snapshot(
            conn, args.task_id,
            run_state_type=rsk.get("state_type"), run_state_name=rsk.get("state_name"),
        )
        if snap is None:
            return _err(f"no such task: {args.task_id}")
        task, comments, events, runs = snap.task, snap.comments, snap.events, snap.runs
        parents, children = snap.parents, snap.children
    ...
    if want_json:
        # ``KanbanTaskSnapshot.to_dict`` is the shared serializer: legacy envelope
        # preserved, plus schema_version / block_kind / block_recurrences /
        # current_run_id / stable comment+event ids.
...
# --- Lifecycle receipts (``--json`` on claim/comment/complete/block/unblock/request-review) ---

def _receipt_capture(args: argparse.Namespace) -> Optional[kb.LifecycleReceiptCapture]:
    """A capture when ``--json`` asked for a receipt, else None (prose path)."""
    return kb.LifecycleReceiptCapture() if getattr(args, "json", False) else None


def _receipt_kwargs(capture: Optional[kb.LifecycleReceiptCapture]) -> dict:
    """``receipt_capture=`` only when a capture exists, so the prose path's DB
    call signature stays byte-identical to the pre-receipt contract."""
    return {"receipt_capture": capture} if capture is not None else {}
...
def _emit_receipt(capture: kb.LifecycleReceiptCapture, operation: str) -> int:
    ...
    violation — surface it as a hard error instead of fabricating a receipt
    the transaction never proved.
    """
    if capture.receipt is None:
        return _err(f"kanban: {operation} succeeded but no receipt evidence was captured")
    print(json.dumps(capture.receipt.to_json_dict()))
    return 0


def _single_id_guard(args: argparse.Namespace, ids: list[str]) -> int:
    """``--expected-run-id`` and ``--json`` attest to exactly one task; refuse the
    ambiguity of a bulk call (rc 2) instead of inventing a bulk envelope."""
    if getattr(args, "expected_run_id", None) is not None and len(ids) != 1:
        return _err("kanban: --expected-run-id cannot be used with multiple task ids", 2)
    if getattr(args, "json", False) and len(ids) != 1:
        return _err("kanban: --json cannot be used with multiple task ids", 2)
    return 0


def _cmd_claim(args: argparse.Namespace) -> int:
    capture = _receipt_capture(args)
    claimer = getattr(args, "claimer", None)
    with kbc.connect_closing() as conn:
        # An explicit ``--claimer`` opts into idempotent replay: a matching live
        # claim returns the original evidence instead of a second run.
        task = kb.claim_task(
            conn, args.task_id, ttl_seconds=args.ttl, claimer=claimer,
            idempotent_replay=claimer is not None, receipt_capture=capture,
        )
        if task is None:
    ...
    if capture is not None:
        return _emit_receipt(capture, "claim")
...
def _cmd_comment(args: argparse.Namespace) -> int:
    ...
    # kanban_command dispatcher maps that to stderr + exit 1, so a stale guard
    # propagates as a real nonzero process exit with zero rows written.
    if_absent = bool(getattr(args, "if_absent", False))
    capture = _receipt_capture(args)
    with kbc.connect_closing() as conn:
        kb.add_comment(
            conn, args.task_id, author, body,
            expected_status=getattr(args, "expected_status", None), if_absent=if_absent,
            **_receipt_kwargs(capture),
        )
    if capture is not None:
        return _emit_receipt(capture, "comment")
    # An --if-absent replay may not have written anything; "ensured" is accurate
    # for both the inserted and the deduplicated outcome.
    print(f"Comment {'ensured on' if if_absent else 'added to'} {args.task_id}")
    return 0
...
def _expected_run_id_for(args: argparse.Namespace, task_id: str) -> Optional[int]:
    """Resolve the run-ownership guard for ``task_id``.

    Without ``--expected-run-id`` this is the worker's env attestation
    (:func:`_worker_run_id_for`), unchanged. With the explicit flag, the env
    attestation (when present) must agree exactly: a foreign
    ``HERMES_KANBAN_TASK``, a run id without its task, a malformed env run id,
    or a differing run id raises ``ValueError`` so the caller exits 2 before
    any DB access — an operator passing an explicit guard inside a worker
    context must not silently override (or be overridden by) that worker's
    attestation.
    """
    ...
    if env_task is not None and env_task != task_id:
        raise ValueError(
            f"--expected-run-id task {task_id!r} does not match HERMES_KANBAN_TASK {env_task!r}"
        )
    if env_run_raw is not None:
        if env_task != task_id:
            raise ValueError("HERMES_KANBAN_RUN_ID is present without a matching HERMES_KANBAN_TASK")
        try:
        ...
        if explicit != env_run:
            raise ValueError(
                f"--expected-run-id {explicit} does not match HERMES_KANBAN_RUN_ID {env_run}"
            )
    return explicit
...
def _cmd_complete(args: argparse.Namespace) -> int:
    ...
    rc = _single_id_guard(args, ids)
    if rc:
        return rc
    capture = _receipt_capture(args)
    try:
        expected_run_id = _expected_run_id_for(args, ids[0]) if len(ids) == 1 else None
    except ValueError as exc:
        return _err(f"kanban: {exc}", 2)
    ...
    with kbc.connect_closing() as conn:
        def refusal(tid: str) -> str:
            """Guard refusals name the guard and the observed status; the
            unguarded path keeps its historical message."""
            if explicit_run_id is None and expected_status is None:
                return f"cannot complete {tid} (unknown id or terminal state)"
            current = kb.get_task(conn, tid)
            actual = current.status if current else "unknown id"
            if explicit_run_id is not None:
                return (f"refusing to complete {tid}: expected status {expected_status!r} "
                        f"and run {expected_run_id!r}, task is {actual!r}")
            return f"refusing to complete {tid}: expected status {expected_status!r}, task is {actual!r}"
        ...
        def op(tid):
            ...
            ok = kb.complete_task(
                ...
                expected_run_id=expected_run_id if len(ids) == 1 else _worker_run_id_for(tid),
                expected_status=expected_status, receipt_capture=capture,
            )
            if not ok:
                fail_msg[tid] = refusal(tid)
            return ok

        if capture is not None:
            # Single id (enforced above): the receipt replaces the prose line.
            if not op(ids[0]):
                return _err(fail_msg[ids[0]])
            return _emit_receipt(capture, "complete")
...
def _explicit_author(args: argparse.Namespace, verb: str, reason: Optional[str]) -> tuple[Optional[str], int]:
    """``--author`` for the transactional reason comment: ``(author, 0)``, or
    ``(None, 2)`` when the flag was given without a usable author/reason pair
    (an attribution with nothing to attribute is a caller error, not a no-op)."""
    explicit = getattr(args, "author", None)
    if explicit is None:
        return None, 0
    author = explicit.strip()
    if not author or not reason:
        return None, _err(f"kanban {verb}: --author requires a non-empty reason and author", 2)
    return author, 0
...
def _cmd_block(args: argparse.Namespace) -> int:
    ...
    expected_status = getattr(args, "expected_status", None)
    explicit_run_id = getattr(args, "expected_run_id", None)
    explicit_author, rc = _explicit_author(args, "block", reason)
    if rc:
        return rc
    author = explicit_author or _profile_author()
    ids = _bulk_ids(args)
    rc = _single_id_guard(args, ids)
    if rc:
        return rc
    ...
    try:
        ...
        return _err(f"kanban: {exc}", 2)
    guarded = expected_status is not None or explicit_run_id is not None
    # Guards, explicit attribution, and --json receipts bind the reason comment
    # to the lifecycle transaction (a receipt's comment_id must be
    # same-transaction evidence). Only legacy unguarded prose calls without an
    # explicit author retain the historical comment-first ordering.
    transactional_reason = bool(reason) and (guarded or explicit_author is not None or as_json)
    ...
    with kbc.connect_closing() as conn:
        ...
        def fail_msg(tid):
            if guarded:
                return f"refusing to block {tid}: {refusals.get(tid) or 'transactional guard refused'}"
            return f"cannot block {tid}"
        ...
        def block(tid):
            result = kb.block_task(
                ...
                with_reason=guarded, **_receipt_kwargs(capture),
            )
            if guarded:
                ok, refusals[tid] = result
                return ok
            return bool(result)

        op = block if transactional_reason else _commented(conn, reason, author, "BLOCKED", block)
        if capture is not None:
            # Single id (enforced above): the receipt replaces the prose line.
            if not op(ids[0]):
                return _err(fail_msg(ids[0]))
            return _emit_receipt(capture, "block")
        return _bulk_apply(ids, op, ok_msg, fail_msg)
...
def _cmd_unblock(args: argparse.Namespace) -> int:
    ...
    explicit_author, rc = _explicit_author(args, "unblock", reason)
    if rc:
        return rc
    author = explicit_author or (_profile_author() if reason else None)
    expected_kind = getattr(args, "expected_block_kind", None)
    # Guards and explicit attribution bind the reason comment to the lifecycle
    # transaction. Only legacy unguarded calls without an explicit author retain
    # the historical comment-first ordering.
    ...
    with kbc.connect_closing() as conn:
        def fail_msg(tid):
            if expected_kind is None:
                return f"cannot unblock {tid} (not blocked/scheduled?)"
            current = kb.get_task(conn, tid)
            actual = f"status={current.status!r} kind={current.block_kind!r}" if current else "unknown id"
            return (f"refusing to unblock {tid}: expected a blocked task with kind "
                    f"{expected_kind!r}, task is {actual}")

        def unblock(tid):
            return kb.unblock_task(
                conn, tid, expected_block_kind=expected_kind,
                reason=reason if transactional_reason else None,
                reason_comment_author=author if transactional_reason else None,
                **_receipt_kwargs(capture),
            )

        op = unblock if transactional_reason else _commented(conn, reason, author, "UNBLOCK", unblock)
        if capture is not None:
            # Single id (enforced above): the receipt replaces the prose line.
            if not op(ids[0]):
                return _err(fail_msg(ids[0]))
            return _emit_receipt(capture, "unblock")
        return _bulk_apply(ids, op, lambda tid: f"Unblocked {tid}{suffix}", fail_msg)
...
def _cmd_request_review(args: argparse.Namespace) -> int:
    ...
    with kbc.connect_closing() as conn:
        ...
        ok, reason = kb.request_review(
            ...
            expected_run_id=expected_run_id, expected_status=getattr(args, "expected_status", None),
            force=bool(getattr(args, "force", False)), with_reason=True, receipt_capture=capture)
        if not ok:
            return _err(f"cannot request review for {tid}: {reason or 'not running/ready?'}")
        if capture is not None:
            return _emit_receipt(capture, "request-review")
