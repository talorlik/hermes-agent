...
def _forward_command(name: str, module: str, attr: str, *, forward_return: bool = False, doc: str = ""):
    ...
    surfaces the return code to ``main()`` (kanban/project/cron propagate).
    """
...
cmd_cron = _forward_command("cmd_cron", "hermes_cli.cron", "cron_command", forward_return=True,
                            doc='Cron job management. Propagates cron_command\'s exit code (detached finalization).')
...
def _prepare_agent_startup(args) -> None:
    ...
    Skipped (after the yolo/safe-mode/user-config env chokepoints) for -z runs
    whose --toolsets precheck already resolved to the explicit-no-tools sentinel
    or a validation error - see ``_oneshot_explicit_no_tools_precheck``."""
    ...
    # env chokepoints above still apply; run_oneshot re-validates and remains
    # the single emitter of the exit-2 message.
