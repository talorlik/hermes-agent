...
# Credential-free updater tests that gate a fork sync push. Deliberately narrow: the
# full suite needs network/credentials and would make every ``hermes update`` slow.
...
def _subprocess_detail(result: subprocess.CompletedProcess) -> str:
...
def _run_fork_sync_tests(cwd: Path) -> tuple[bool, str]:
    ...
    try:
        probe = subprocess.run(
            [python, "-c", "import pytest, pytest_asyncio"], cwd=cwd, timeout=30, **_GIT_TEXT_KW)
        if probe.returncode != 0:
            ...
            install = subprocess.run(
                [str(uv_bin), "pip", "install", "--python", python, "pytest==9.1.1", "pytest-asyncio==1.3.0"],
                cwd=cwd, timeout=5 * 60, **_GIT_TEXT_KW)
            if install.returncode != 0:
                return False, _subprocess_detail(install)
        result = subprocess.run(
            [python, "-m", "pytest", *_FORK_SYNC_TEST_PATHS, "-q"], cwd=cwd, timeout=15 * 60, **_GIT_TEXT_KW)
...
def _rollback_fork_sync_candidate(git_cmd: list[str], cwd: Path, rollback_ref: str) -> bool:
    ...
    rollback_result = subprocess.run(git_cmd + ["reset", "--hard", rollback_ref], cwd=cwd, **_GIT_TEXT_KW)
    ...
    if (rollback_result.stderr or "").strip():
...
def _validate_fork_sync_candidate(git_cmd: list[str], cwd: Path, rollback_ref: str) -> bool:
    ...
    Imported lazily from ``update_cmd`` so test patches on that module stay effective.
    """
...
def _fork_sync_strategy() -> str:
    ...
    "ff_only" (default): only sync the fork when origin/main is strictly behind upstream/main
    (pure fast-forward, the historical behavior). "merge": when the fork carries its own
    commits, merge upstream/main into main instead of skipping, then push the result to origin.
    Unknown values fall back to "ff_only" - a typo must never enable an automatic merge the
...
def _push_fork_after_sync(git_cmd: list[str], cwd: Path, *, verb: str) -> None:
    print("→ Syncing fork...")
    if _sync_fork_with_upstream(git_cmd, cwd):
        print("  ✓ Fork synced with upstream")
    else:
        print(
            f"  ℹ {verb} upstream but couldn't push to fork (no write access?)\n"
            "    Your local repo is updated, but your fork on GitHub may be behind."
        )


def _merge_upstream_into_fork(git_cmd: list[str], cwd: Path, *, origin_ahead: int, upstream_ahead: int) -> bool:
    """``updates.fork_sync_strategy: merge`` path: merge upstream/main into main, validate, push.

    Local commits are preserved in a merge commit; a conflict aborts cleanly with nothing changed;
    a ``pre-upstream-sync-<stamp>`` tag is left as a human-visible recovery anchor and the pre-merge
    SHA drives the automatic rollback when the candidate fails validation.
    """
    from hermes_cli.update_cmd import _capture_head_sha

    print(f"\n→ Fork has {origin_ahead} commit(s) of its own and is {upstream_ahead} commit(s) behind upstream")
    ...
    subprocess.run(git_cmd + ["tag", sync_tag], cwd=cwd, capture_output=True, check=False)
    merge_result = subprocess.run(git_cmd + ["merge", "--no-edit", "upstream/main"], cwd=cwd, **_GIT_TEXT_KW)
    if merge_result.returncode != 0:
        subprocess.run(git_cmd + ["merge", "--abort"], cwd=cwd, capture_output=True, check=False)
        print("  ✗ Could not merge upstream/main (conflict or dirty tree) — sync stopped, nothing was changed.")
    ...
    _push_fork_after_sync(git_cmd, cwd, verb="Merged")
    return True
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path, *, assume_yes: bool = False, input_fn=None) -> bool:
    ...
    When the fork carries its own commits, behavior is governed by ``updates.fork_sync_strategy``:
    "ff_only" (default) skips with a notice; "merge" merges upstream/main into main (see
    ``_merge_upstream_into_fork``). Either way the candidate is syntax-checked and the targeted
    updater tests run before anything is pushed; a failure rolls back to the pre-sync HEAD.
    ...
    from hermes_cli.update_cmd import (
        _capture_head_sha, _count_commits_between, _has_upstream_remote, _no_prompt_git_kwargs,
    ...
    if origin_ahead < 0 or upstream_ahead < 0:
        ...
        return False
    if origin_ahead > 0:
        # Don't trample fork-only commits unless the user opted into automatic merges.
        ...
        if upstream_ahead == 0:
            ...
            return True
        return _merge_upstream_into_fork(git_cmd, cwd, origin_ahead=origin_ahead, upstream_ahead=upstream_ahead)
    if upstream_ahead == 0:
        print("  ✓ Fork is up to date with upstream")
        return True
    ...
    if not _validate_fork_sync_candidate(git_cmd, cwd, pre_sync_sha):
        print("  Try the sync again once the candidate passes validation.")
        return False
    _push_fork_after_sync(git_cmd, cwd, verb="Got updates from")
    return True
