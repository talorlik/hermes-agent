...
def build_cron_parser(subparsers, *, cmd_cron: Callable) -> None:
    ...
    cron_create.add_argument("--script",
        help="Path to a script under ~/.hermes/scripts/. Default mode: "
            ...
            "else via Python.")
    cron_create.add_argument("--script-failure-policy",
        choices=["continue", "fail_closed"], default="continue",
        help="Behavior when an agent pre-run script fails: continue injects the "
    ...
    cron_edit.add_argument(
        "--script-failure-policy", choices=["continue", "fail_closed"], default=None,
        help="Set agent pre-run script failure behavior. fail_closed requires an "
    ...
    cron_incidents.add_argument("--state",
    ...
    cron_finalize = cron_subparsers.add_parser(
        "finalize-detached", help="Finalize a detached cron run by correlation id")
    cron_finalize.add_argument("run_id", help="Correlation id from the RUN_STARTED directive")
    finalize_result = cron_finalize.add_mutually_exclusive_group(required=True)
    finalize_result.add_argument("--success", action="store_true", help="Report the run succeeded")
    finalize_result.add_argument("--failed", action="store_true", help="Report the run failed")
    cron_finalize.add_argument("--error",
