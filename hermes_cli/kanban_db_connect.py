...
Also owns the transaction-outcome contract that sits on top of that
boundary: fail-closed ``in_transaction`` probing, provable rollback
(:class:`TransactionOutcomeUnknownError` when closure cannot be proven), the
consistent-read ``read_txn``, and the lifecycle-receipt frame stack that lets
mutators stage evidence inside a write transaction and publish it only after
the outer COMMIT succeeds (:class:`LifecycleReceipt` /
...
# ---------------------------------------------------------------------------
#
# Every boundary below treats the connection's own telemetry as untrusted:
# instrumentation wrappers routinely omit ``in_transaction`` or expose a
# look-alike attribute, and an interrupt can land between a boundary statement
# executing inside SQLite and its Python wrapper returning. A rollback that is
# not PROVEN closed therefore escalates to ``TransactionOutcomeUnknownError``
# instead of being re-raised as an ordinary "rolled back" failure.
...
# ---------------------------------------------------------------------------
#
# A receipt is the row-id evidence (run / event / comment) a lifecycle verb
# produced inside its write transaction. Mutators STAGE a receipt against the
# connection's current frame; ``write_txn`` PUBLISHES staged receipts into the
# caller's capture only after the outer COMMIT (and its durability check)
# succeeded, so a caller can never observe evidence for a mutation that was
# rolled back or whose durability is unproven.
...
# Keyed by ``id(conn)``: one frame per open write_txn level on that connection.
...
def _begin_receipt_transaction(
    ...
    with _RECEIPT_TXN_LOCK:
        ...
        # A savepoint nested under a raw (caller-managed) BEGIN inherits that
        # frame's non-owner status: nothing below it may ever publish.
...
def write_txn(conn: sqlite3.Connection, *, allow_nested: bool = False):
    ...
    ``Exception``: a ``KeyboardInterrupt`` between a staged receipt and the
    commit boundary must still roll back the savepoint/transaction and pop the
    receipt frame, otherwise a caller that catches the interruption inherits an
    open transaction, a bound capture, and a leaked receipt frame. The original
    exception is always re-raised; an unprovable rollback escalates to
    :class:`TransactionOutcomeUnknownError` (see the outcome contract above).
    """
    ...
    try:
        ...
        try:
            # Post-commit torn-extend check: header page_count must match the
            # actual file pages. Do not publish a success receipt when
...
def read_txn(conn: sqlite3.Connection):
    ...
    readers like ``build_task_snapshot`` can run under an outer ``read_txn``
    or ``write_txn`` unchanged.
    """
