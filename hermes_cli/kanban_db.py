...
# --- Durable text canonicalization (domain boundary) ---
...
def _validate_expected_status(expected_status: Optional[str]) -> None:
    """Reject an ``expected_status`` guard value outside :data:`VALID_STATUSES`.

    An unknown guard value would never match any row and read as a mismatch;
    a typo'd caller must fail loudly instead of being silently refused.
    """
...
def add_comment(
    ...
    ``if_absent`` makes the write idempotent for retrying callers (the Dream
    sync loop replays ``comment --expected-status running`` after crashes):
    when an existing comment on this task already has this exact author and
    byte-identical canonical (stripped) body, the existing comment id is
    returned and nothing is written -- no comment row, no ``commented`` event.
    The status guard is validated FIRST, so a stale ``expected_status`` fails
    even for an exact replay: idempotent success must not mask that the
    guarded precondition no longer holds. The dedup read runs inside the same
    write transaction as the insert, so a concurrent duplicate writer
    serializes behind ``BEGIN IMMEDIATE`` and the replay observes its
    committed row.
    """
    ...
    _validate_expected_status(expected_status)
    # Domain-boundary canonicalization (canonical-return policy): safe bodies
    # stay byte-identical, secret-bearing bodies are masked before the dedup
    # read below, so an ``if_absent`` replay of unsafe text matches the stored
    # canonical form instead of inserting twice.
    body = canonicalize_durable_text(body)
    now = int(time.time())
    # ``allow_nested=True``: graph builders (kanban_swarm blackboard seeding)
    # and the guarded block/unblock reason comments compose comment writes
    # under one outer commit.
    with write_txn(conn, allow_nested=True):
            ...
            if existing is not None:
                ...
                _stage_receipt(
                    ...
                    LifecycleReceipt(
                        ...
                        newly_committed=False, comment_id=int(existing["id"]),
                    ),
                )
        ...
        event_id = _append_event(conn, task_id, "commented", {"author": author, "len": len(body)})
        _stage_receipt(
            conn, receipt_capture,
            LifecycleReceipt(
                operation="comment", task_id=task_id,
                prior_status=row["status"], final_status=row["status"],
                newly_committed=True, event_id=event_id, comment_id=int(cur.lastrowid or 0),
            ),
        )
...
def _append_event(
    ...
    """Insert an event row inside the caller's txn and return its id; ``run_id``
    groups it by attempt (NULL = task-scoped). The id is the lifecycle-receipt
    evidence for the transition that emitted the event."""
    cur = conn.execute(
...
def _claim_and_open_run(
    ...
    *, event_extra: Optional[dict] = None,
    ...
    """CAS ``source_status -> running``, open a run row, emit ``claimed``; returns
    ``(run_id, claimed_event_id)`` or None when the CAS lost. Caller holds the
    txn. Both ids are returned because they are the receipt evidence for the
    claim: the run row and the event that recorded it."""
    cur = conn.execute(
        f"""
        UPDATE tasks
           SET status        = 'running',
...
def _matching_live_claim_replay(
    conn: sqlite3.Connection, task_id: str, lock: str, now: int,
) -> Optional[int]:
    """Run id of an authoritative, unexpired ``running`` claim held by ``lock``.

    Every condition is checked against both the task row and its current run
    row so a stale pointer cannot be replayed: the run must belong to this
    task, still be ``running`` with no ``ended_at``, carry the same lock, and
    share the exact ``claim_expires`` the task row advertises. Anything else
    (foreign lock, expired claim, runless task, mismatched run identity)
    returns None so the caller refuses instead of pretending to own the claim.
    """
    ...
    if (
        ...
        return int(current["current_run_id"])
    return None
...
def claim_task(
    ...
    """
    _clear_receipt_capture(conn, receipt_capture)
    if idempotent_replay and (not isinstance(claimer, str) or not claimer.strip()):
    ...
    with write_txn(conn):
        if idempotent_replay:
            replay_run_id = _matching_live_claim_replay(conn, task_id, lock, now)
            if replay_run_id is not None:
                if receipt_capture is not None:
                    # Authoritative replay evidence: the ORIGINAL run row and
                    # its ``claimed`` event (linked by run_id), read inside the
                    # same write transaction that validated the live claim. A
                    # run without its event is not evidence -- refuse.
                    ev = conn.execute(
                    ...
                    _stage_receipt(
                        ...
                        LifecycleReceipt(
                            ...
                            newly_committed=False, run_id=replay_run_id, event_id=int(ev["id"]),
                        ),
                    )
                return get_task(conn, task_id)
            # A running task that is NOT our live claim falls through to the
            # ordinary CAS, which loses (status != 'ready') and returns None.
        ...
        opened = _claim_and_open_run(conn, task_id, "ready", lock, expires, now)
        if opened is None:
            return None
        run_id, event_id = opened
        # prior_status is a transactional fact: the CAS UPDATE matched exactly
        # one row WHERE status = 'ready'.
        _stage_receipt(
            conn, receipt_capture,
            LifecycleReceipt(
                operation="claim", task_id=task_id, prior_status="ready", final_status="running",
                newly_committed=True, run_id=run_id or None, event_id=event_id,
            ),
        )
...
def complete_task(
    ...
    expected_status: Optional[str] = None, fire_lifecycle_hook: bool = True,
    receipt_capture: Optional[LifecycleReceiptCapture] = None,
) -> bool:
    ...
    ``created_cards`` are verified inside the write transaction — a phantom id
    records an auditable event (committed with the transaction) and then
    raises :class:`HallucinatedCardsError`; afterwards the prose is scanned
    for unresolvable ``t_<hex>`` refs (advisory event only).

    ``expected_status`` is a compare-and-swap guard for callers that read the
    status and then decide to complete (TOCTOU window — e.g. a caller that
    observed ``running`` while the task has since moved to ``blocked``). When
    supplied, the completion only fires if the status still equals it inside
    the write transaction; on a mismatch the function returns False with no
    side effects. Both guards (``expected_status`` and ``expected_run_id``) are
    evaluated BEFORE the created-card audit, so a stale caller cannot leave a
    ``completion_blocked_hallucination`` event behind; they are conjunctive.
    """
    _clear_receipt_capture(conn, receipt_capture)
    now = int(time.time())
    _validate_expected_status(expected_status)
    ...
    metadata = canonicalize_durable_value(metadata) if metadata is not None else None
    normalized_expected_run_id = int(expected_run_id) if expected_run_id is not None else None
    ...
    with write_txn(conn):
        ...
        # must refuse before ANY side effect, including the phantom-card
        # audit event below. write_txn is BEGIN IMMEDIATE, so these reads are
        # atomic with the UPDATE that follows.
        prior = conn.execute(
        ...
        if normalized_expected_run_id is not None and prior_run_id != normalized_expected_run_id:
            return False
        ...
        # completion still needs an auditable event, so it is recorded here
        # and committed with the rest of this transaction; the
        ...
        verified_cards, phantom_cards = _gate_created_cards(conn, task_id, created_cards, summary or result)
        if not phantom_cards:
            ...
            _stage_receipt(
                conn, receipt_capture,
                LifecycleReceipt(
                    operation="complete", task_id=task_id, prior_status=prior_status,
                    final_status="done", newly_committed=True, run_id=run_id, event_id=event_id,
                ),
            )
    if phantom_cards:
        # The audit event above is durable (the transaction committed on clean
        # exit); reject the completion only now.
...
def _gate_created_cards(
    ...
    """Verify ``created_cards`` inside the caller's write txn; returns
    ``(verified_ids, phantom_ids)``. A phantom id is recorded as a
    ``completion_blocked_hallucination`` event in that same transaction (the
    audit trail commits with it) without touching task state; the caller
    raises :class:`HallucinatedCardsError` once the transaction has closed so
    the rollback-on-exception path cannot erase the audit event."""
    ...
    return verified_cards, phantom_cards
...
def edit_completed_task_result(
    ...
    # Domain-boundary canonicalization (canonical-return policy): the backfill
    # writes the same durable completion surfaces as complete_task.
    result = canonicalize_durable_text(result) if result is not None else None
    summary = canonicalize_durable_text(summary) if summary is not None else None
    metadata = canonicalize_durable_value(metadata) if metadata is not None else None
    handoff_summary = summary if summary is not None else result
...
def block_task(
    ...
    expected_status: Optional[str] = None, reason_comment_author: Optional[str] = None,
    with_reason: bool = False, receipt_capture: Optional[LifecycleReceiptCapture] = None,
):
    ...
    event, no lifecycle hook — regardless of routing branch. Conjunctive with
    ``expected_run_id``: both must match.

    ``reason_comment_author`` makes the reason comment part of THIS write
    transaction: when supplied together with ``reason``, the ``BLOCKED:
    <reason>`` comment is inserted (via :func:`add_comment`'s nested
    savepoint) after the transition/event and before the lifecycle hook on
    every routing branch, so a failed comment write rolls back the entire
    transition and suppresses the hook. This is SQLite atomicity only — it
    makes no claim about filesystem or external hook side effects. When
    omitted, no comment is written here (the CLI's unguarded path keeps its
    historical comment-first ordering outside the transaction).
    ...
    The lifecycle hook fires post-commit on EVERY routing branch (including
    ``dependency`` -> ``todo``): a failed or rolled-back commit fires no hook.
    """
    _clear_receipt_capture(conn, receipt_capture)
    ...
    # lands on the event payload, the closed/synthesized run summary, and the
    # transactional ``BLOCKED:`` comment — mask secrets once here.
    ...
    with write_txn(conn):
        ...
        # CAS guards first: a stale ``expected_status``/``expected_run_id`` must
        # refuse before any routing branch runs its UPDATE / run closure /
        # events / hook. write_txn is BEGIN IMMEDIATE, so these reads and the
        # UPDATE below are atomic with respect to concurrent writers — the
        # refusal reasons are transactional facts, not post-hoc re-reads.
        if expected_status is not None and cur_row["status"] != expected_status:
            return _ret(False, f"expected status {expected_status!r}, task is {cur_row['status']!r}")
        ...
        if cur_row["status"] not in ("running", "ready"):
            return _ret(
                False,
                f"task is not in a blockable state (running/ready); task is {cur_row['status']!r}",
            )
        ...
        # Same-transaction reason comment, BEFORE the lifecycle hook: a failed
        # comment write must abort the transition without the hook ever having
        # fired (the hook fires post-commit, in the shared tail below).
        ...
        _stage_receipt(
            conn, receipt_capture,
            LifecycleReceipt(
                operation="block", task_id=task_id, prior_status=cur_row["status"],
                final_status=new_status, newly_committed=True, run_id=run_id,
                event_id=event_id, comment_id=comment_id,
            ),
        )
        # Fresh task read inside the transaction; the hook itself fires only
        # after COMMIT succeeds, on EVERY routing branch.
...
def request_review(
    ...
    ``expected_run_id``, parent gating). A mismatch refuses before the status
    flip, run closure, and the ``review_requested`` event.
    """
    _clear_receipt_capture(conn, receipt_capture)
    ...
    with write_txn(conn):
        ...
        if trow is None:
            return _ret(False, "task not found")
        # CAS guard: refuse a stale status observation before the claim rules
        # and the UPDATE below. write_txn is BEGIN IMMEDIATE, so this read is
        # atomic with the transition it gates.
        if expected_status is not None and trow["status"] != expected_status:
            return _ret(False, f"expected status {expected_status!r}, task is {trow['status']!r}")
        ...
        _stage_receipt(
            conn, receipt_capture,
            LifecycleReceipt(
                operation="request_review", task_id=task_id, prior_status=trow["status"],
                final_status="review", newly_committed=True, run_id=run_id, event_id=event_id,
            ),
        )
    return _ret(True)
...
def unblock_task(
    conn: sqlite3.Connection, task_id: str, *, expected_block_kind: Optional[str] = None,
    reason: Optional[str] = None, reason_comment_author: Optional[str] = None,
    receipt_capture: Optional[LifecycleReceiptCapture] = None,
) -> bool:
    """``blocked``/``scheduled`` -> its resumable phase (parent re-gated; ``review``
    when that is where it left off), closing any leaked run first.

    ``expected_block_kind`` is a compare-and-swap guard for callers that read
    the block's typed cause and then decide to unblock. When supplied, the
    transition requires — inside the same write transaction — that the task
    is currently ``blocked`` AND that the authoritative stored ``block_kind``
    equals it exactly. Only :data:`VALID_UNBLOCK_EXPECTED_KINDS` are accepted:
    ``dependency`` waits live in ``todo`` (never ``blocked``), so asserting
    that kind is a category error and raises :class:`ValueError` instead of
    reading as a mismatch. A legacy un-typed block (``block_kind IS NULL``)
    never matches, and ``scheduled`` tasks are refused (they carry no block
    ...
    unblock. SQLite atomicity only — no filesystem/external claims.
    """
    _clear_receipt_capture(conn, receipt_capture)
    # An unknown guard value would never match any row and read as a mismatch;
    # a typo'd caller must fail loudly instead. ``dependency`` is rejected here
    # too — see VALID_UNBLOCK_EXPECTED_KINDS.
    if expected_block_kind is not None and expected_block_kind not in VALID_UNBLOCK_EXPECTED_KINDS:
        raise ValueError(
            f"expected_block_kind must be one of {sorted(VALID_UNBLOCK_EXPECTED_KINDS)}"
        )
    # Domain-boundary canonicalization (canonical-return policy): the reason
    ...
    with write_txn(conn):
        ...
        if expected_block_kind is not None and (
            current is None
            or prior_status != "blocked"
            or current["block_kind"] != expected_block_kind
        ):
            return False
        resume_status = (
            _resume_status_from_events(conn, task_id) if prior_status == "blocked" else "ready"
        )
        _reclaim_dangling_run(
        ...
        if cur.rowcount != 1:
            return False
        # Same-transaction reason comment (guarded CLI path): a failed write
        # here rolls back the status flip and the event below.
        comment_id: Optional[int] = None
        if reason and reason_comment_author:
            comment_id = add_comment(conn, task_id, reason_comment_author, f"UNBLOCK: {reason}")
        payload: Optional[dict] = (
        ...
        if reason:
            # Persist the operator's reason as durable audit evidence on the
            # event itself, not only in the (deletable) comment stream.
            payload = {**(payload or {}), "reason": reason}
        event_id = _append_event(conn, task_id, "unblocked", payload)
        _stage_receipt(
            conn, receipt_capture,
            LifecycleReceipt(
                operation="unblock", task_id=task_id, prior_status=prior_status,
                final_status=new_status, newly_committed=True, event_id=event_id, comment_id=comment_id,
            ),
        )
        return True
...
# Legacy ``show --json`` task envelope (byte-compatible key set and order) ...
_TASK_SNAPSHOT_LEGACY_FIELDS = (
...
# ... plus the additive authoritative lifecycle fields (schema_version 1).
_TASK_SNAPSHOT_LIFECYCLE_FIELDS = ("block_kind", "block_recurrences", "current_run_id")
_SNAPSHOT_COMMENT_FIELDS = ("id", "author", "body", "created_at")
_SNAPSHOT_EVENT_FIELDS = ("id", "kind", "payload", "created_at", "run_id")
...
def _fields_dict(obj: Any, fields: tuple[str, ...]) -> dict[str, Any]:
...
class KanbanTaskSnapshot:
    ...
    def to_dict(self) -> dict[str, Any]:
        task_d = _fields_dict(self.task, _TASK_SNAPSHOT_LEGACY_FIELDS + _TASK_SNAPSHOT_LIFECYCLE_FIELDS)
        # Legacy normalization: NULL and [] both serialize as [].
        task_d["skills"] = list(self.task.skills) if self.task.skills else []
        return {
            ...
            "comments": [_fields_dict(c, _SNAPSHOT_COMMENT_FIELDS) for c in self.comments],
            "events": [_fields_dict(e, _SNAPSHOT_EVENT_FIELDS) for e in self.events],
            "runs": [_fields_dict(r, _SNAPSHOT_RUN_FIELDS) for r in self.runs],
        }


def build_task_snapshot(
    conn: sqlite3.Connection, task_id: str, *,
    run_state_type: Optional[str] = None, run_state_name: Optional[str] = None,
    ...
    run-state time-travel filter; all other collections are always complete
    (no truncation — surface-level caps are the consumer's decision, applied
    ...
    with read_txn(conn):
        ...
        return KanbanTaskSnapshot(
            ...
            runs=list_runs(conn, task_id, state_type=run_state_type, state_name=run_state_name),
...
from hermes_cli.kanban_db_connect import (  # noqa: E402
    ...
    init_db,
    write_txn,
    # Transaction-outcome contract + lifecycle receipts. The frame stack and
    # ``write_txn`` are one mechanism, so they live together in
    # ``kanban_db_connect``; the lifecycle verbs below stage receipts through
    # these names. ``_RECEIPT_TXNS`` is re-exported as the same dict object so
    # invariant checks against ``kanban_db._RECEIPT_TXNS`` observe live state.
    # Boundary fault-injection points (``_execute_boundary_with_retry``,
    # ``_check_file_length_invariant``) are deliberately NOT re-exported:
    # ``write_txn`` resolves them in ``kanban_db_connect``, so that is the only
    # namespace where patching them has an effect.
