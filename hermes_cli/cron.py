...
def _job_rows(job: Dict[str, Any]) -> List[tuple[str, str]]:
    ...
    optional = [
        ...
        # Label must fit the 11-column ``{label + ':':<11}`` layout in cron_list (max 10 chars
        # incl. colon); "Script policy" overflows and glues the value to the label. It sits
        # directly under the Script row, so the short label is unambiguous.
        ("Policy", job.get("script_failure_policy", "continue") if job.get("script") else ""),
...
def cron_runs(job_id: Optional[str] = None, limit: int = 20):
    ...
    for record in records:
        ...
        if record.get("delivery_target"):
            print(f"    delivery={record.get('delivery_status', '?')} "
        ...
        if record.get("detached_run_id"):
            print(f"    detached run={record['detached_run_id']} "
...
def cron_finalize_detached(args) -> int:
    ...
    Exit codes: 0 finalized (or idempotent repeat of the same terminal report), 1 conflicting
    rewrite refused, 2 unknown correlation id. Failure evidence is force-redacted at the ledger
    layer before it persists.
    """
    ...
    if record is not None:
        print(f"Finalized detached run {run_id}: {record.get('detached_status')} "
    ...
    if current == desired:
        print(f"Detached run {run_id} already finalized as {current} (idempotent no-op)")
        return 0
    print(f"Detached run {run_id} is already {current!r}; refusing to rewrite it as {desired!r}")
    return 1
