...
def _positive_run_id(value: str) -> int:
    """Argparse validator for an explicit run-ownership guard (``--expected-run-id``)."""
    try:
        run_id = int(value)
    except (TypeError, ValueError) as exc:
        raise argparse.ArgumentTypeError("--expected-run-id must be a positive integer") from exc
    if run_id <= 0:
        raise argparse.ArgumentTypeError("--expected-run-id must be a positive integer")
    return run_id


# Compare-and-swap guards shared by the lifecycle verbs. Each verb documents
# its own scope in the help text; the flag names are identical on purpose so
# a guarded caller (e.g. the Dream sync loop) uses one vocabulary everywhere.
def _expected_status(verb: str, extra: str = ""):
    return _arg("--expected-status", choices=sorted(kb.VALID_STATUSES),
                help=f"Only {verb} if the task still has this exact status (compare-and-swap "
                     f"guard against racing status changes).{extra}")


def _expected_run_id(verb: str):
    return _arg("--expected-run-id", type=_positive_run_id,
                help=f"Only {verb} if this exact positive owning run is still current.")


def _receipt_json(extra: str = ""):
    return _json_flag(help=f"Print a versioned lifecycle receipt (JSON) instead of prose.{extra}")


_SINGLE_ID_RECEIPT = " Single task id only."
...
_SPECS = [
    ...
    _cmd("comment", [
        ...
        _expected_status("comment"),
        _arg("--if-absent", action="store_true",
             help="Idempotent write: exit 0 without adding anything if this exact author+body "
                  "comment already exists on the task (retry-safe for guarded sync loops)."),
        _receipt_json(),
    ...
    _cmd("complete", [
        ...
        _expected_status("complete", " Applies to every listed task id."),
        _expected_run_id("complete"),
        _receipt_json(_SINGLE_ID_RECEIPT),
    ...
    _cmd("block", [
        ...
        _expected_status("block", " Applied independently to every listed task id; a mismatch "
        ...
        _expected_run_id("block"),
        _receipt_json(_SINGLE_ID_RECEIPT + " Binds the reason comment to the block transaction."),
    ...
    _cmd("unblock", [
        ...
        _arg("--expected-block-kind", choices=sorted(kb.VALID_UNBLOCK_EXPECTED_KINDS),
             help="Only unblock if the task is blocked with exactly this typed block kind "
                  "(compare-and-swap guard). Applied independently to every listed task id; a "
                  "mismatch refuses that id before its reason comment is written."),
        _arg("--author", help="Explicit author for the transactional UNBLOCK reason comment."),
        _receipt_json(_SINGLE_ID_RECEIPT + " Binds the reason comment to the unblock transaction."),
        _TASK_IDS,
    ...
    _cmd("request-review", [
        ...
        _expected_status("request review", " Conjunctive with all existing claim/run rules."),
        _expected_run_id("request review"),
        _receipt_json(),
