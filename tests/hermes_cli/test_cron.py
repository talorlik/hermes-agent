...
class TestCronCommandLifecycle:
    ...
    def test_list_displays_script_failure_policy(self, tmp_cron_dir, capsys):
        ...
        # cron list renders aligned ``label:<pad>value`` rows (upstream layout); the
        # policy row sits directly under the Script row.
        out = capsys.readouterr().out
        assert re.search(r"Script:\s+gate\.py\n\s+Policy:\s+fail_closed\n", out), out
