...
def _run_update_until_guard(args, tmp_path):
...
def test_venv_holder_guard_force_semantics(
    force, force_venv, expected, capsys, tmp_path
):
    result = _run_update_until_guard(
        _update_args(force=force, force_venv=force_venv), tmp_path
    )
