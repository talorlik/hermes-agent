...
class TestTransactionOutcomeContract:
    ...
    def test_outer_rollback_interrupt_raises_outcome_unknown(
        ...
        try:
            ...
            assert isinstance(excinfo.value, kbc.TransactionOutcomeUnknownError)
    ...
    def test_release_cleanup_interrupt_preserves_original_and_unbinds(
        ...
        original_method = getattr(kbc.LifecycleReceiptCapture, cleanup_method)
        ...
        try:
            ...
            first = kbc.LifecycleReceiptCapture()
            second = kbc.LifecycleReceiptCapture()
            monkeypatch.setattr(
                kbc.LifecycleReceiptCapture, cleanup_method, interrupt_once
            )
            with pytest.raises(BaseException) as excinfo:
                with kbc.write_txn(conn):
                    kb.add_comment(
                    ...
                    raise ValueError("original body failure")
            monkeypatch.setattr(
                kbc.LifecycleReceiptCapture, cleanup_method, original_method
            )
            assert state["fired"]
    ...
    def test_publish_cleanup_second_interrupt_keeps_finalization_error(
        ...
        original_publish = kbc.LifecycleReceiptCapture._publish
        original_clear = kbc.LifecycleReceiptCapture.clear
        ...
        try:
            ...
            second = kbc.LifecycleReceiptCapture()
            monkeypatch.setattr(
                kbc.LifecycleReceiptCapture, "_publish", interrupt_publish
            )
            monkeypatch.setattr(
                kbc.LifecycleReceiptCapture, "clear", exit_first_cleanup_clear
            )
            with pytest.raises(BaseException) as excinfo:
                with kbc.write_txn(conn):
                    kb.add_comment(
            ...
            monkeypatch.setattr(
                kbc.LifecycleReceiptCapture, "_publish", original_publish
            )
            monkeypatch.setattr(
                kbc.LifecycleReceiptCapture, "clear", original_clear
            )
            ...
            assert isinstance(excinfo.value, kbc.ReceiptFinalizationError)
            assert isinstance(excinfo.value.__cause__, KeyboardInterrupt)
            notes = "\n".join(getattr(excinfo.value, "__notes__", []))
    ...
    def test_ordinary_durability_failure_is_outcome_unknown(
        ...
        try:
            ...
            with pytest.raises(kbc.TransactionOutcomeUnknownError) as excinfo:
...
class TestOwnerTelemetryFailClosed:
    ...
    def test_wrapper_without_telemetry_fails_closed(
        ...
        try:
            ...
            assert id(wrapper) not in kbc._RECEIPT_TXNS
            assert id(conn) not in kbc._RECEIPT_TXNS
            assert capture.receipt is None
            assert capture._active_connection_id is None
    ...
    def test_raising_telemetry_read_completion_chains_cleanup_failure(
        ...
        try:
            ...
            assert not isinstance(
                excinfo.value.__cause__, kbc.TransactionOutcomeUnknownError
            )
