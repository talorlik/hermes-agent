...
def test_recently_finished_long_running_execution_survives_retention(
    ...
    """Retention ranks terminal rows by ``finished_at`` (upstream 83efdf5e5e), not
    ``claimed_at``: an execution claimed long ago but finished just now is the
    NEWEST evidence and must outlive a sibling that finished earlier.

    Adapted to the per-job / 30-day-floor retention model: both attempts belong
    to the same job so the per-job cap applies, and ``claimed_at`` is backdated
    past ``RETENTION_DAYS`` so the floor does not shield either row. Under a
    ``claimed_at`` ordering the long-running row would rank second and be
    evicted; under the correct ``finished_at`` ordering the earlier-finished
    sibling is the one pruned."""
    executions = _point_ledger(monkeypatch, tmp_path)
    monkeypatch.setattr(executions, "MAX_TERMINAL_EXECUTIONS", 1)
    long_running = executions.create_execution("same-job", source="builtin")
    assert executions.mark_execution_running(long_running["id"]) is not None
    newer = executions.create_execution("same-job", source="builtin")
    assert executions.finish_execution(newer["id"], success=True) is not None

    # Age both claims beyond the retention floor so the cap is what decides.
    ...
    try:
        conn.execute(
            "UPDATE executions SET claimed_at = ? WHERE id IN (?, ?)",
            (ancient, long_running["id"], newer["id"]),
        )
        conn.commit()
    finally:
        conn.close()
